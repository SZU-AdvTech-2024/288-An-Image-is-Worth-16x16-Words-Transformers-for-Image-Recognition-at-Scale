...
features = pca.fit_transform(features)[:, :10]
...
# Z = linkage(mean_features, method='ward')
# cluster_order = fcluster(Z, t=1.5, criterion='distance') - 1  # 根据距离阈值调整聚类的数量
# mean_features_sorted = mean_features[cluster_order.argsort()]
# 绘制热图
plt.figure()
sns.heatmap(mean_features, annot=False, cmap="coolwarm", xticklabels=[f"PC{i+1}" for i in range(10)], yticklabels=[folder_mapping[i] for i in range(8)])
